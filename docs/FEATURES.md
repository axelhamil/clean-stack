...
- **TanStack Router file-based via `virtualRouteConfig`** — `apps/app/routes.ts` declares the whole tree once, each leaf pointing at its `<name>.route.tsx` (route + page in one file, `autoCodeSplitting: true` chunks it as long as the component stays unexported). Layouts/gates as individual files under `apps/app/src/router/`. `apps/app/src/router.tsx` is a thin `createRouter({ routeTree })`; `routeTree.gen.ts` is generated by the Vite plugin and gitignored. `defaultPreload: "intent"` prefetches on hover.

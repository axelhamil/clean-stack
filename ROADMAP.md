...
| 0.7 | Railway reference deploy (api + app + Postgres + storage + cron sweep, env baseline, `main` → auto-deploy) | **CODE DONE, PROD VALIDATION IN-PROGRESS** — May 2026. Config-as-code SOTA 2026 livré (`infra/railway/{api,app,cron}.toml`, runbook `docs/DEPLOY-RAILWAY.md`, cron entrypoint `apps/api/src/cron/sweep.ts`, prod.Dockerfile no-bundle pattern Bun-natif après fix thread-stream/pino worker). **Deploy actuel = api healthcheck `/livez` fail sur Railway** — root cause à investiguer (probable env var missing au boot OU PORT mismatch). Reste à valider end-to-end : (1) debug logs api Railway, (2) smoke-test complet runbook §10 (sign-up → email → org → upload R2 → RGPD outbox+audit → Sentry release), (3) cron manual trigger. À reprendre. Closes Phase 0 une fois `/livez` 200 sur prod. |

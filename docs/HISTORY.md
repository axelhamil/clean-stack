...
4. **SSO enforcement covers all four sign-in surfaces, not just the password form** — `/sign-in/email`, `/sign-up/email`, `/sign-in/magic-link` are checked in `hooks.before` (`isSsoEnforcedFor` against the request's email, before the rate-limit branch — which `return`s for `/sign-in/email`, so anything placed after it would never run). Passkey sign-in carries no email, so it can't be checked path-based; it's closed separately in `databaseHooks.session.create.before`, keyed off whether the session's linked account already belongs to a registered SSO provider (an SSO-originated session must never be blocked by the guard meant to keep *other* paths out). All four return `403 { message: "SSO_REQUIRED", providerId }` — confirmed live that `providerId` survives JSON serialization, so the front never re-derives the provider from the email.
...
8. **Every SCIM/SSO event payload carries `actorUserId`, including SCIM's bearer-token-only requests** — SCIM endpoints authenticate with a token, not a session (`ctx.context.session` is empty), so the actor for `scim.*` events is resolved from the connection row (`scimConnectionOwner`, the user who generated the token) rather than from session state. Pre-delete snapshots (`ssoProviderDeleteSnapshots`, `scimConnectionDeleteSnapshots`, keyed on `providerId`) supply `organizationId` for the `*.deleted`/`*.connection.deleted` events, since the row is already gone by the time `hooks.after` runs.
...
**Known, pre-existing, out-of-scope flake observed while verifying the above**: `interaction.spec.ts`'s "traps focus inside the command palette" test fails intermittently (`getByRole('dialog', { name: 'Command Palette' })` not found) — reproduced across multiple runs at multiple worker counts, and confirmed present in the pre-fix-round build too, so it predates this task and is unrelated to any change here. Not investigated further; flagged for separate follow-up.

...
mock.module("../../di/container", () => ({
  di: {
    GdprService: { processPendingDeletions: mockedExecute },
  },
}));

const { internalRoutes } = await import("../internal.routes");
...
describe("POST /internal/gdpr-sweep", () => {
  beforeEach(() => {
  ...
  describe("when the request carries a valid signature", () => {
    it("should pass through to the use case and return its output", async () => {
      ...
      const headers = await signedHeaders("POST", "/internal/gdpr-sweep", body);
      const res = await makeApp().request("/internal/gdpr-sweep", {
        method: "POST",
        headers,
    ...
    it("should forward the validated body fields to the use case", async () => {
      ...
      const headers = await signedHeaders("POST", "/internal/gdpr-sweep", body);
      await makeApp().request("/internal/gdpr-sweep", {
        method: "POST",
        headers,
        body: JSON.stringify(body),
      });
    ...
    it("should fall back to the env default batchSize when omitted", async () => {
      ...
      expect(mockedExecute).toHaveBeenCalledWith({
        batchSize: env.GDPR_SWEEP_BATCH_SIZE,
        dryRun: false,
      });
    });
  });

...
- **All four enforced sign-in paths**: `/sign-in/email`, `/sign-up/email`, and `/sign-in/magic-link` were exercised live against an SSO-enforced domain and each returned `403 { message: "SSO_REQUIRED", providerId }`. The **passkey path was verified by code inspection only** — `databaseHooks.session.create.before` is a single hook shared by every session-creation path (including the three above, which never reach it because they're already blocked earlier), and there was no WebAuthn authenticator available to drive a real ceremony in this environment (the same category of gap accepted in earlier tasks of this phase).

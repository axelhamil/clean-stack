...
- [x] **`notification-map.ts`** — third projection of the catalog after `visibility-map` (webhooks) and `retention-map` (purge). Absent event = no notification (the default: most of the 65 events are audit-only). `forced: true` short-circuits preferences *and* batching, before either is evaluated (the SOTA critical bypass).
...
- [x] **No new events.** D.3 consumes the catalog, doesn't extend it — a notification is a read projection of an already-audited event, and `notification.created` would loop with its own subscriber. Catalog stays **65 / 28 public / 37 internal**.

...
| **Phase C.2 — Operator audit log (cross-org read + hash chain + `/admin` zone)** | **Jul 2026** | `GET /admin/audit-log` (cursor pagination, filters actor/org/target/actionPrefix/date) + `GET /admin/audit-log/verify` — gated `requirePlatformAdmin` (`PLATFORM_ADMIN_IDS` allowlist OR `role === "admin"`, MFA optionnelle via `PLATFORM_ADMIN_REQUIRE_MFA`). Tamper-evidence hash chain env-gatée (`AUDIT_TAMPER_EVIDENCE`, SHA-256 + advisory lock, genesis-at-activation, zero backfill). Meta-audit `security.operator.audit_accessed` (compliance, première page seulement) → event #48 (absorbé dans le recount C.5). Front : zone pathless `/admin` (`ensurePlatformAdmin` via `customSession.isPlatformAdmin`) + `features/admin-audit-log/` (infinite query, filtres, `MetadataSheet` diff before/after, `ChainBadge`) + nav operator (pill AppShell + groupe palette). As-built in [`docs/HISTORY.md`](docs/HISTORY.md). |
...
| **Phase C.6 — Account recovery codes UI** | **Jul 2026** | `RecoveryCodesCard` on `/settings/account` (regenerate-only, password gate, codes natively formatted `xxxxx-xxxxx` by BetterAuth, copy + download `clean-stack-recovery-codes.txt`). Backup-code fallback on `/two-factor` (input normalization tolerant: whitespace stripped, dash auto-inserted on 10-char input). `BackupCodeUsedNotifier` (first `onEvent` handler: on-use email via `ResendEmailService`, template `backup_code_used`, dev logs instead of sending). Rate-limit on `generate-backup-codes` (5 req/900s, fail-closed). 2 new compliance events: `user.mfa.backup_codes_regenerated` (payload: `{userId}`) + `user.mfa.backup_code_used` (payload: `{userId, email}`) → **54 total / 50 subscribable / 4 internal**. NIST 800-63B-4 look-up secrets posture (no entropy gain on server hash). Encrypted-not-hashed deviation documented (BetterAuth default kept). As-built in [`docs/HISTORY.md`](docs/HISTORY.md). |
| **Phase A.5 — Privacy dashboard** | **Jul 2026** | `/settings/privacy` UX hub: `<PolicyAcceptanceCard />` + `<ConsentSettings />` + `<DataSourcesCard />` + `<DataExportCard />` + `<SessionsCard />`. Danger zones contextual (GitHub/Linear/Vercel model): `<RgpdDeletionCard />` → bottom of `account.page.tsx`; org leave/delete → bottom of `organization.page.tsx`. `sub-processors.config.ts` promoted to `apps/app/src/shared/`. Danger tab dissolved. 0 events, catalog stays 54/50/4. As-built in [`docs/HISTORY.md`](docs/HISTORY.md). |
| **Phase D.5 — Email delivery queue** | **Aug 2026** | `email_message` durable queue + `EmailDeliveryWorker` (poll 2 s, claim 300, chunk 100, `resend.batch.send`) + `@packages/emails` in-repo React Email templates (`TEMPLATE_IDS` as override) + port methods `sendRaw`/`sendTemplateBatch`/`sendRawBatch` + `tx?` option + retention sweep `EMAIL_MESSAGE_RETENTION_DAYS=7` + `email.delivery.exhausted` internal event → **55 total / 50 subscribable / 5 internal**. Provider facts: rate limit 10 req/s per team; `scheduled_at` supported in batch; bounce suppression provider-side and automatic (no `email_suppression` table needed). As-built in [`docs/HISTORY.md`](docs/HISTORY.md). |
...
- [x] `apps/app/src/shared/sub-processors.config.ts` — typed const `SUB_PROCESSORS` (interface `SubProcessor { name, purpose, region, category, url?, dpaUrl?, status }`). Active: Resend, Cloudflare R2, BetterAuth OAuth. Planned: Stripe, GrowthBook, Umami. (Relocated from `features/legal/` to `shared/` in A.5 — consumed by both the legal page and `DataSourcesCard`.)
...
- [x] Linked via `command-palette.tsx` (2 `LEGAL_ROUTES` entries) + `data-rights.page.tsx` cross-links. Footer link deferred (no global footer yet).
...
> **Status — shipped (Jul 2026).** `/settings/privacy` UX hub wired. Six decisions: (1) GitHub-style contextual danger zones — no Danger tab; delete-account at bottom of Account page, org leave/delete at bottom of Organization page (SOTA: GitHub/Linear/Vercel). (2) `PolicyAcceptanceCard` = status only (accepted version + up-to-date badge) driven by existing `policiesQueryOptions`; acceptance history deferred (data in `policy_acceptance` table, no new back-end route). (3) Direct RGPD download link dropped — presigned key discarded after email is sent; export stays email-only; refactor-only preserved. (4) `DataSourcesCard` = static list of active `SUB_PROCESSORS`, no per-user last-sync timestamp (no substrate). (5) `sub-processors.config.ts` promoted to `shared/` — two route-owning features (`legal/` + `privacy/`) cannot import each other per import-direction rule. (6) ROADMAP deviation: `<RgpdDeletionCard />` went to `account.page.tsx` (danger zone) instead of Privacy page. 0 events, catalog stays **54 total / 50 subscribable / 4 internal**. As-built in [`docs/HISTORY.md`](docs/HISTORY.md).
...
- [x] `/settings/privacy` page (`apps/app/src/features/privacy/privacy.{route,page}.tsx`) — composes: `<PolicyAcceptanceCard />` (A.2 acceptance status) + `<ConsentSettings />` (A.4) + `<DataSourcesCard />` (A.3 sub-processors static list) + `<DataExportCard />` (rgpd, relocated from `account.page.tsx`) + `<SessionsCard />` (security, relocated from `account.page.tsx`). `<RgpdDeletionCard />` relocated to bottom of `account.page.tsx` (contextual danger zone) — deviation from original spec. `<ActiveSessionsCard>` = `<SessionsCard>` (same component).
- [x] `/settings/account` slims down: `<SessionsCard />` + `<DataExportCard />` moved to Privacy; `<RgpdDeletionCard />` + `<Link to="/legal/data-rights">` remain at the bottom as a contextual danger zone; Passkeys/2FA/RecoveryCodes cards unchanged.
- [x] Danger tab (`apps/app/src/features/danger/`) dissolved: `org-danger-card.tsx` + `transfer-leave-dialog.tsx` moved to `apps/app/src/features/organization/components/`, rendered at bottom of `organization.page.tsx`.
- [x] Add Privacy tab to `SETTINGS_TABS` (`requiresOrg: false`, personal scope); Danger entry removed from `SETTINGS_TABS` + command-palette.
...
> **Status — shipped (Jul 2026).** Repurposed org-admin → **operator** surface during design: the read API is gated `requirePlatformAdmin` (cross-org), **not** `requireOrgPermission({ auditLog: ["read"] })` — the statement stays in `@packages/access-control` for a future per-tenant view. Tamper-evidence promoted from deferred to shipped (env-gated `AUDIT_TAMPER_EVIDENCE`, genesis-at-activation, zero backfill) + verify endpoint + meta-audit event `security.operator.audit_accessed`. As-built + decisions in [`docs/HISTORY.md`](docs/HISTORY.md).

- [x] Page `/admin/audit-log` (pathless `adminLayout` + `ensurePlatformAdmin`) — filters (actor, org, target, action prefix, date range), cursor pagination, `MetadataSheet` before/after diff, `ChainBadge`.
- [x] **Tamper-evidence** — `prevHash`/`hash`/`sequence` + SHA-256 chain calc behind `AUDIT_TAMPER_EVIDENCE`, `GET /admin/audit-log/verify`.

...
- **No user-visible text is a literal at its call site.** Every string that reaches the DOM, a `toast()`, an `aria-label` or an email subject comes from a catalog key through `t()`. **The three documented exceptions**: the untranslated legal prose — the policy bodies in `features/legal/policies/` and the prose sitting directly in the `accessibility`, `cookies`, `data-rights` and `sub-processors` routes — which is per-locale content a clone-owner replaces (translating a ToS is a legal act, not a dev task) and which **every one of those pages discloses in the reader's own language** through `UntranslatedBodyBanner`, because what makes untranslated prose acceptable is the disclosure, never the omission; `EVENT_DESCRIPTIONS` in `@packages/events`, which stays English with the rest of the API documentation; and the sub-processor register (`apps/app/src/shared/sub-processors.config.ts`), which stays English because it restates the signed DPA — though the two fields a user actually reads off it, `purpose` and `region`, are translated, through `common.legal.subProcessors.*` behind `shared/sub-processor-labels.ts`. **Why**: a literal is invisible to both the type-checker and the parity gate, so it can never be translated and its absence can never be detected — a defect no CI run will ever surface. **Test before merging**: search the diff for a quoted string sitting in JSX text, in a `label`/`placeholder`/`title`/`aria-label` attribute, or in a `toast` argument. If one is there and it isn't one of the three exceptions above, it is wrong. English is the source of truth: add the key to `packages/i18n/src/catalogs/en/` first, or `tsc` rejects the call site. **One carve-out that is not an exception**: `authorization-devtool.tsx` keeps English literals — it is dev-only, gated behind `import.meta.env.DEV` and never reaches a user's DOM.
- **Extending the translated surface is all-or-nothing per screen.** Translating half a page is worse than translating none of it: a French card sitting above an English one reads as a bug, not as work in progress. When a phase picks up a feature, its cards, its dialogs, its forms and the toasts its hooks raise move together, and the parity gate then holds both locales in step. A feature directory and a screen are not the same unit — `/settings/privacy` drew its cards from three directories, which is how one of them stayed English through E.1a.

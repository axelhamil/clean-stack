...
- **Mutations skip plain `Error`s without a `status` field** (`isUnexpectedMutationError`). **Why**: real API failures always carry `status` (via `throwApiError`) and real network failures are `TypeError`s — a bare `Error` thrown inside a mutationFn is a flow-control signal (`new Error("Cancelled")` on passkey cancel, `EMAIL_NOT_VERIFIED_REDIRECT`), and `MutationCache.onError` fires *before* the hook's local `onError` can swallow it. Throwing flow-control errors from mutations stays allowed because of this rule; if you need a captured custom mutation error, attach a `status`.

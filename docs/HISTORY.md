...
**Sentry: reasoned from code, not observed live.** `apps/api/.env` carries a `SENTRY_DSN` key with an empty value; `env.ts` normalizes every empty-string env var to `undefined` before Zod sees it, so `env.SENTRY_DSN` is `undefined` and `di.ts`'s `env.SENTRY_DSN ? new SentryInstrumentation() : new NoOpInstrumentation()` binds `NoOpInstrumentation`. No live Sentry project was available in this environment, so the trace shape described in [`docs/CRON.md`](docs/CRON.md)'s "What a sweep looks like in tracing" — one `sweep` span, a `db.query` lease-acquire child, one `sweep.pass` child per pass carrying `stop_reason`, a `db.query` child per batch — is asserted from `sweepSpans()`'s implementation and its call sites in `sweep-runner.ts` and `sweep-purge.ts`, not from an opened Sentry trace. What **was** confirmed directly: with `SENTRY_DSN` unset, `NoOpInstrumentation` is what `di.ts` binds, so every sweep above ran with instrumentation fully disabled and still worked — Sentry off does not change sweep behavior.

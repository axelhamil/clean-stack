...
| 0.6 | Retention sweeps (`outbox_event` / `audit_log` / `webhook_delivery`) | DONE — 3 routes `/internal/sweep-*`, SOTA 2026 defaults. Chained entrypoint `apps/api/src/cron/sweep.ts` shipped at 0.7 (bundled to `dist/cron/sweep.js`). |
| 0.7 | Railway reference deploy (api + app + Postgres + storage + cron sweep, env baseline, `main` → auto-deploy) | DONE — May 2026. Config-as-code SOTA 2026 (`infra/railway/{api,app,cron}.toml`, "$schema" pinned), 3 services + Postgres add-on EU + R2 default storage. Cron = service Railway dédié réutilisant l'image api (`bun dist/cron/sweep.js` @ 03:17 UTC, `restartPolicy=NEVER`, no GH Actions fallback — user-decided single-source). 2 prod.Dockerfile fixes in-scope (HEALTHCHECK `/health`→`/livez`, suppression `pnpm --filter @packages/* run build` qui violait rule §4 internal-packages-ship-source). `.env.example` audité avec markers `REQUIRED IN PROD`. `.github/workflows/deploy.yml` trigger Railway webhook per service (matrix api/app/cron) sur push `main`. Runbook complet `docs/DEPLOY-RAILWAY.md` (Shared Variables, R2 vs Railway Bucket calc comparatif, smoke-test, removability Fly/Render/Cloud Run). Closes Phase 0. Unblocks B.1 + D.1. |

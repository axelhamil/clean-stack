...
**`requireCurrentPolicies` mounted for real — reversing Phase A.2 decision 5.** A.2 shipped the middleware composable and unmounted, reasoning that a global mount "would make every current API call return 409 for a stale user — too aggressive." H.1 reverses that: a user who hasn't accepted the current policy version now gets 409 on business routes, API tokens included. Not mounted globally — four exclusion classes, each for a stated reason: `POST /me/policies/accept` and `GET /me/policies` (the gate would otherwise block the only route that clears it), RGPD Art. 17/20 routes (a data-subject right cannot be conditioned on accepting terms), sign-out and session reads (a user must always be able to leave), and impersonated sessions (`session.impersonatedBy` set — C.3 already skips the legal gate on the front during impersonation; the server had to agree or an operator would get 409-walled out of the account they're inspecting). Mounting it on `/api/v1` needed a typing change, not a second middleware: that surface runs on `ApiTokenVariables`, not `AuthVariables`, so `requireCurrentPolicies` was retyped against the minimal contract it actually reads — `{ user: SessionUser; session?: { impersonatedBy?: string | null } }` — making it mountable on both surfaces. A token request has no session, so the impersonation bypass is simply never taken there, which is correct: a token cannot impersonate.

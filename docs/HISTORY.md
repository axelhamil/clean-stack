...
- **`SendTemplateOptions.locale?`** — slot reserved for the i18n phase. Adapter logs a warn ("not yet implemented") if passed; resolution will switch to `${template}_${locale}` env lookup when locale-prefixed templates land. *Note: removed rather than activated by the i18n foundation (Phase E.1a) — a batch-level locale would force one language on every recipient of a batch, so locale moved onto `EmailRecipient<K>.locale` instead. See that entry.*
...
## i18n foundation — typed catalogs, locale outside the URL ✅ Phase E.1a · Aug 2026

**Why**: the ROADMAP originally prescribed Lingui and `/en/...`/`/fr/...` locale routes. Both were re-decided on evidence before writing any code — see [`docs/superpowers/specs/2026-08-29-e1a-i18n-foundation-design.md`](superpowers/specs/2026-08-29-e1a-i18n-foundation-design.md) for the full design record this section summarizes.

**`i18next` replaces Lingui.** Measured 2026-08-29: `i18next` at 21.7M and `react-i18next` at 15.7M weekly downloads against Lingui's 1.6M and Paraglide's 0.5M — a 31× adoption gap against the stated "known and recognized" requirement. Its key typing is also the strongest of the three, the opposite of the received wisdom: `ParseKeys` (`i18next/typescript/t.d.ts`) constrains `t()` across nested keys, namespaces, `keyPrefix`, `context` and the six plural categories; a plural key requires `{ count: number }`; interpolation types from the format specifier. Lingui's Typed Message IDs only work in explicit-ID mode, not with its default macros, and `react-intl` generates no types at all. `@vitejs/plugin-react` no longer runs Babel for the JSX transform (`oxc-transform-react` does it now) — Lingui's macro would have dragged Babel back into a build that just left it; i18next is pure runtime, no build-time extraction step. Costs accepted: ~23.4 kB gzip of runtime (i18next 13.4 + react-i18next 10.0) against Lingui's ~3.7 kB and Paraglide's ~0.5 kB, and catalogs are not tree-shaken (a dead key still ships). `i18next` itself has zero runtime dependencies since v26.0.7; MIT license on both packages keeps a fork available if ever needed.

**Catalogs are `.ts … as const`, never `.json`.** Probed directly against the repo's `typescript@7.0.2`: with `resolveJsonModule` on (already the case, `packages/typescript-config/base.json`), TypeScript widens every JSON string value to `string` — assigning an unrelated string to `typeof catalog["greeting"]` compiles with no error. The identical catalog written as a `.ts` module with `as const` correctly rejects it (`TS2322`). Since `CustomTypeOptions.resources` is bound to `typeof enCatalog` for the `t()` key-checking guarantee, a `.json` catalog would silently defeat the entire premise of this design — an unknown or mistyped key would type-check. Keys and plural suffixes (`_one`/`_other`) survive either format since they're property names, but only the `.ts` form protects the values, which is where interpolation placeholders (`{{name}}`) live. `packages/i18n/src/load-catalog.ts`'s `Resources` type widens every string-literal leaf of the English catalog's shape (`WidenStrings<T>`) so a French catalog can share the same structure without being forced to the exact English literals — that widening is scoped to the runtime resources bag only; `CustomTypeOptions.resources` in `types.ts` stays bound to the literal English catalog, so the `t()` guarantee is untouched.

**`SendTemplateOptions.locale` was removed, not activated.** An early draft assumed the port's shared `options` bag was where locale belonged. It isn't: `sendTemplateBatch(template, recipients[], options)` takes N recipients under one `options`, so a batch-level locale would force a single language onto every recipient in the batch — and the two genuinely multi-recipient callers (`flush-notification-emails.route.ts`'s one-digest-per-user job and `rgpd.service.ts`'s export/deletion notices) are exactly the ones that must not be forced. Locale instead moved onto the recipient entry itself, `EmailRecipient<K>.locale`, defaulting to `DEFAULT_LOCALE` per recipient in `QueuedEmailService.sendTemplateBatch` and rendered through `renderTemplate(template, variables, locale)` before the row is written — the resolved language is frozen onto the `email_message.locale` column at enqueue time, so a queue retry days later still renders in the language the recipient had when the email was requested, not whatever locale they've since switched to.

**Partial translation is a stated, accepted state — not an oversight.** E.1a extracted `auth`, `account` and the app shell into catalogs; `admin`, `webhooks`, `sso`, `billing`, `organization`, the rest of `settings`, and the legal pages remain hardcoded English, tracked as E.1b in the ROADMAP. This is safe only because `createI18n` always registers the English catalog under `DEFAULT_LOCALE` **in addition to** the requested locale's resources — `fallbackLng: DEFAULT_LOCALE` is otherwise inert, since i18next can only fall back to a language whose resources are actually loaded into the instance. The result: a French user gets French navigation, sign-in/sign-up, account settings, error messages and email subjects, around English content on the unextracted deeper pages — a mixed-language UI, never a raw `"settings.billing.title"` key rendered to the screen. `packages/i18n/src/__tests__/parity.test.ts` enforces exact key parity between `en` and `fr` for every namespace that does exist, so the partial coverage is a scope decision, not a silently-drifting one.

**Two decisions recorded but deliberately out of scope**: `EVENT_DESCRIPTIONS` (`packages/events/src/event-descriptions.ts`) and the `/developers/events` page stay English permanently, consistent with the rest of the API documentation surface. And the pre-existing `apps/app/src/shared/CLAUDE.md` claim that `ThemeProvider`'s CSP nonce comes from a Caddy-injected `meta[property="csp-nonce"]` tag was found, while auditing `app-providers.tsx`, to reference a tag `index.html` doesn't actually contain — the nonce is always `undefined` in practice. Left unfixed here: out of this phase's scope, noted so it's a known debt rather than a silent discovery.

---

## Email delivery queue — debt closeout ✅ Aug 2026

**Why**: Phase D.5 shipped with three known debts (`failed` rows never purged, `markSent` one UPDATE per row, RGPD deletion confirmation missing its idempotency key). Closing them surfaced two review findings that widened the phase beyond those three: a live production defect in the provider-side dedup key, and a second, more serious loss mode in the RGPD path that the original debt list hadn't named.

**The design draft's central claim was wrong, and it matters**: the draft justified skipping a dedicated fix by arguing the `email_message.idempotency_key` UNIQUE constraint already closed the duplicate-email window. It doesn't. The constraint protects an **INSERT** — two enqueue calls racing to create the same row. The actual failure mode is a **re-send of an already-inserted row**: the worker crashes after the provider accepts the send but before `markSent` commits, the row is re-claimed on the next poll, and `sendChunk` calls Resend again for a row that was never marked `sent`. No INSERT occurs on that second call, so the UNIQUE constraint never sees it. What actually closes that window is the provider-side `Idempotency-Key` header on the batch request — Resend recognizes the key and returns the cached response instead of sending twice. That header was itself broken (next paragraph), so for a period the window this design believed was closed, wasn't, by any mechanism.

**Finding 1 — silent mail loss via key truncation.** `chunkIdempotencyKey` built the provider's `Idempotency-Key` by joining every recipient key in the chunk and truncating to 256 characters. Two structurally different chunks (different recipients, different content) can share the same first 256 characters of their joined keys, producing the same truncated header. Resend then returns the **cached response of the first send** for the second, structurally different chunk — and `sendChunk` reads that cached response as success and marks its own (different) recipients `sent`, even though nothing was ever sent to them. This was a live production defect, not a theoretical one: any two chunks colliding on the truncated prefix silently dropped mail while the queue reported success. Fixed by hashing the joined keys instead of truncating them, so no two distinct key-sets can collide onto the same header.

**Finding 2 — permanent loss of the RGPD deletion confirmation on a crash.** The confirmation email was enqueued *after* the wipe transaction committed. A crash (or any failure) between the commit and the enqueue call permanently lost the confirmation — the wipe is one-shot and never replayed, so there was no second chance to send it. Fixed by moving the enqueue inside the wipe transaction itself, with `throw new Error("rollback")` on enqueue failure — a failed enqueue now rolls back the wipe rather than completing it silently without notifying the account holder.

**The three originally-scoped debts, also closed**:

- **`enqueue` idempotency at the queue layer**: gained `onConflictDoNothing` targeted at the `idempotency_key` unique index, plus `returning()` — a warning is logged when fewer rows come back than were requested, surfacing silent conflict-driven drops instead of hiding them.
- **Per-recipient idempotency keys**: `EmailRecipient.idempotencyKey` lets a caller set a key per recipient; the positional fallback (`${options.idempotencyKey}#${index}`) is namespaced with `#` and shared by both the template batch path and the raw batch path. **This is a one-time cross-deploy dedup window, not a cosmetic rename**: the separator moved from `/` to `#`, so every existing single-recipient caller's stored key changes shape (`<key>/0` → `<key>#0`). A row enqueued before the deploy and retriggered after it will not match its old key and will not dedupe — one duplicate email per affected key, bounded by `EMAIL_MESSAGE_RETENTION_DAYS=7` (old rows age out of the dedup window within a week). The digest flush route already keyed its send on a digest of the full row-id list (`digestIdempotencyKey(rows.map(r => r.id))`), unchanged by this pass — no per-call timestamp was ever involved there.
- **`failed` rows never purged**: `EMAIL_MESSAGE_FAILED_RETENTION_DAYS` (default 90) plus a new partial index (`email_message_failed_sweep_idx`) backs a second sweep pass. `sent` rows still purge on the original `EMAIL_MESSAGE_RETENTION_DAYS=7`. The cutoff is measured from `created_at` (enqueue time), not from when the row transitioned to `failed` — since the retry ceiling is minutes, the two are close in practice, but a row enqueued 89 days ago that fails today is purged on the very next sweep.
- **`markSent` fan-out cost**: collapsed from one `UPDATE` per row to a single `UPDATE … WHERE id IN (...)` (Drizzle `inArray`) with a `CASE WHEN` selecting each row's `provider_message_id` — a 100-message chunk now costs one round-trip instead of 100.

**Supporting changes**: `runRetentionSweep` was promoted from a single pass to N passes, and six route files were migrated onto the promoted signature: the five direct `runRetentionSweep` callers, plus `sweep-audit-log`, which had been bypassing the shared runner via `runBatchedSweep` on its own. All six now share the same runner as the two new email sweep passes (`sent`, `failed`), rather than growing a bespoke second code path.

**Rule #6 amended, not exempted**: closing the `failed`-row debt meant no new domain event fires when the sweep deletes a `failed` row — but rule #6's "infra retention sweeps" exception only named `outbox_event`, `audit_log`, `webhook_delivery`. `email_message` wasn't on that list, and invoking the exception as written anyway would have stretched a rule's scope silently, which rule #1 forbids. The exception list was rewritten to add `email_message`, with the justification spelled out: a row reaches `status = 'failed'` only when `nextAttempt.isNone()` — the exact point `email.delivery.exhausted` is already emitted — so every purged `failed` row corresponds 1:1 to an event already emitted and audited before the sweep ever runs.

**No new event types.** The catalog is consumed here, not extended — verified by importing `packages/events/src/visibility-map.ts` and counting `Object.values(VISIBILITY)`: **81 total / 35 public / 46 internal**, unchanged from Phase E.1a.

**New debt found and deliberately left open**: the sweep routes and the cron client (`internalLayers`, `Bun.serve`, `signedInternalFetch`) have no explicit request timeout anywhere in the chain — no `AbortSignal` on the outbound `fetch`, no `idleTimeout` on the server. Doubling the number of sweep passes (one more for `email_message` `failed` rows, on top of the existing runners) brings a hang closer to actually mattering, but wiring timeouts through the whole internal-call path was out of scope for a queue-correctness closeout. Tracked as **D.5** in the ROADMAP's "What's left" table rather than folded in silently.

---

## Sweep rail instrumentation ✅ Phase D.6 · Aug 2026

**Why**: the debt above closed a queue-correctness gap but left the whole `/internal/sweep-*` rail dark to tracing — a batch stuck on `lock_timeout`, a lease that never releases, or a pass silently truncating on `MAX_BATCHES` all looked identical from Sentry: nothing. The spec named three components that had to land together (the runner, the six `purgeBatch` bodies, `sweep_lock`) because instrumenting only one of them — the lease, say — produces a trace with a hole exactly where the run and its batches happened.

**Façade over constructor injection — a deliberate rule #8 deviation.** Rule #8 asks for `IInstrumentation` on the constructor; this rail has none to put it on — the runner, the lease helpers and the purge routine are plain functions, not a class. `SweepSpans` (`apps/api/src/shared/internal-routes/sweep-span.ts`) is built once per request from `di.IInstrumentation` and threaded down explicitly instead, which keeps both properties the rule protects: no module-level singleton (two sweep labels can run concurrently without sharing a span budget), and a unit test builds one over `NoOpInstrumentation` and spies on its calls the same way a constructor-injected mock would.

**Span budget: `MAX_INSTRUMENTED_BATCHES = 50` against `MAX_BATCHES = 1000`.** Sentry truncates a transaction at roughly 1000 spans by dropping the overflow *from the end* — the exact spans most worth keeping (the last passes, the lease release) would be the first discarded if every batch opened one. `sweepSpans()` counts down a budget and, once exhausted, still executes the query — the trace loses resolution, never work. A full-backlog run therefore traces its first 50 batches and executes the remaining 950 unwrapped; verified by `apps/api/src/shared/internal-routes/__TESTS__/sweep-span.test.ts` (5/5) and by inspection, not by driving a 1000-batch backlog through a real database.

**Double-capture avoidance.** Two error paths in `sweep-runner.ts` were previously silent and are now instrumented, but not the same way: a batch failure under `onBatchError: "break"` and a failed lease release are both swallowed **by design** (a sweep that already did useful work must not fail the whole request over them) and are now the only two paths that call `spans.capture(...)` directly. A batch failure with no `onBatchError` still rethrows, and is deliberately left uncaptured at that point — `app.onError` already reports every uncaught exception, and capturing it twice would double-count it in Sentry.

**`purgeBatch` promotion — justified by rule #2 and by what it enables, not by jscpd.** Six routes carried byte-identical `purgeBatch` bodies; the plan expected promoting them to `purgeBatchWithTimeout` (`apps/api/src/shared/internal-routes/sweep-purge.ts`) to lower jscpd's duplication numbers. **It does not.** A worktree comparison with the same binary and config measured the repo before and after this branch: **484 duplicated lines / 3042 tokens / 35 clones before, 496 / 3107 / 35 after** — duplication went slightly up, not down, and stayed well under the 3% gate (~1.24% both times, exit 0 either way). The reason is specific to jscpd's exact-token matcher: it never flagged the six `purgeBatch` bodies at all, because each route names different tables and columns — only the 3-line `SET LOCAL` boilerplate ever matched, and 3 lines is below the 10-line clone threshold. What jscpd flags, before and after, is the six routes' near-identical **import blocks**, which grew because all six now import `di`, `purgeBatchWithTimeout` and `sweepSpans`. The promotion is still correct: repo rule #2 (2nd occurrence promotes) applied at three occurrences already, and it is what turned "instrument the batched delete" into a one-file change instead of six.

**Verification against a real database (the point of this task).** Postgres up, schema pushed, dev user seeded, API started with `bun --hot`, then the bundled cron (`apps/api/src/cron/sweep.ts`) run against it with a real `INTERNAL_SIGNING_KEY`. All six sweeps (`sweep-email-messages`, `sweep-webhook-delivery`, `sweep-audit-log`, `sweep-outbox`, `sweep-consents`, `sweep-notifications`) answered `OK` with `deleted: 0` (fresh database), every pass's `stopReasons` `exhausted`, no `SKIPPED`/`FAIL`/`UNREACHABLE` line, cron exit code 0. This is the only step in the whole plan that exercises `purgeBatchWithTimeout`'s query against a real Postgres — every unit test in Tasks 1–6 runs against a mocked `@packages/drizzle`, which type-checks a query but cannot execute it.

**Sentry: reasoned from code, not observed live.** `apps/api/.env` carries a `SENTRY_DSN` key with an empty value; `env.ts` normalizes every empty-string env var to `undefined` before Zod sees it, so `env.SENTRY_DSN` is `undefined` and `di.ts`'s `env.SENTRY_DSN ? new SentryInstrumentation() : new NoOpInstrumentation()` binds `NoOpInstrumentation`. No live Sentry project was available in this environment, so the trace shape described in [`docs/CRON.md`](docs/CRON.md)'s "What a sweep looks like in tracing" — one run span (`op: "function"`), a `db.query` lease-acquire child, one pass span (`op: "function"`) per pass carrying `stop_reason`, a `db.query` child per batch — is asserted from `sweepSpans()`'s implementation and its call sites in `sweep-runner.ts` and `sweep-purge.ts`, not from an opened Sentry trace. What **was** confirmed directly: with `SENTRY_DSN` unset, `NoOpInstrumentation` is what `di.ts` binds, so every sweep above ran with instrumentation fully disabled and still worked — Sentry off does not change sweep behavior.

**Final whole-branch review, live under a real Sentry SDK (Aug 2026).** A follow-up review ran the real `runRetentionSweep` under `@sentry/bun` with a stub transport instead of reasoning from code, and found two defects the unit suite could not see. First, the lease's two spans shared the same budget counter as the batch and dry-run-count spans; because the lease release runs last (in the runner's `finally`), any run past ~48 batches dropped `delete from sweep_lock` from the trace — reproduced live at budget 4, where the release span was simply absent. Fixed by giving the lease an unbudgeted `spans.lease(...)` path (`sweep-span.ts`'s `dbSpan` helper, shared by both `db` and `lease`), on the reasoning that the budget exists to protect exactly this span, so spending it on the lease defeats its own purpose. Second, the run span itself carried no attributes — a run skipped for a held lease was indistinguishable in Sentry from a run that executed and found nothing. Fixed by writing `sweep.skipped: true` on the early return and `sweep.deleted`/`sweep.batch_count`/`sweep.truncated` on the completed path, both while the run span is still active. Both fixes are covered by tests: a lease span surviving an exhausted budget (`sweep-span.test.ts`), and the run-span attributes on both paths (`sweep-runner.test.ts`).

**Sentry's ~1000-span truncation — now measured, not assumed.** The original span-budget note above (and the matching claims in `docs/CRON.md`, `docs/FEATURES.md`, `ROADMAP.md`, and `sweep-span.ts`'s docstring) asserted that Sentry truncates a transaction at ~1000 spans by dropping the overflow *from the end*. The plan that produced those docs had explicitly flagged this as unconfirmed in the installed SDK and said nothing should claim it as fact — that caveat did not survive into the shipped text. The same live-Sentry review closed the gap: emitting 1200 spans in one transaction retained exactly 1000 (`b0`..`b999`), with the tail dropped. The claim was true; it is now backed by a measurement instead of an assumption.

**Two facts about earlier tasks worth recording here.** First, Task 3 found a pre-existing bug already on `dev`: `sweep-notifications.test.ts` failed in isolation with `Export named 'sweepSchema' not found` (introduced in `88e3e92`) but passed in the full suite, because bun's `mock.module` registry is process-global and another test file's factory happened to supply the export first. The shared `drizzle-mock.ts` factory this plan extracted removes that whole class of trap by giving every sweep test the same, complete mock. Second, the plan's Task 5 expected `pnpm check:sweep-lock` to pass at that point; it could not, because that script drives the real routes, which were deliberately left non-functional from Task 4 until Task 6 wired the runner, the purge helper and the lease together. The gate moved to Task 6, where it passes 23/23, and stayed 23/23 through Task 7.

**No new event types.** Nothing here mutates persistent business state beyond what the existing sweeps already purge — catalog stays **81 total / 35 public / 46 internal**, unchanged from Phase E.1a and the email queue debt closeout.

---

## i18n extraction — retiring the partial-translation carve-out ✅ Phase E.1b · Aug 2026

**Why**: E.1a shipped the typed rail and translated `auth`, `account` and the shell; the rest of the product — `admin`, `webhooks`, `sso`, `billing`, `organization`, the rest of `settings`, every `/legal/*` page — stayed hardcoded English, tracked as E.1b. This phase closed that list, then retired the `apps/app/src/features/CLAUDE.md` hedge that had named the state as deliberate.

**Namespace split — `admin` gets its own catalog, not a home inside `settings`.** `packages/i18n/src/catalogs/en/settings.ts` runs to 520 lines; `admin.ts` sits at 152. Folding the admin/audit-log/org-admin copy into `settings.ts` would have made one namespace carry two audiences the app already keeps structurally apart — `apps/app/CLAUDE.md`'s import-direction rule forbids one route-owning feature importing another for exactly this reason, and it's what forced Task 13's `role-labels.ts` promotion to `shared/auth` rather than leaving it in `features/organization`. A separate `admin` namespace (`useTranslation("admin")`) means a customer-facing settings screen can never accidentally resolve platform-operator copy, and the reverse.

**R2 — i18next `_one`/`_other` plurals, no new ICU primitive.** Ruled before dispatch: the repo already shipped 20 plural keys on this pattern, so introducing an ICU message-format primitive for this phase's own new counts (API-token expiry in days and years, webhook delivery-attempt counts) would have been a second plural mechanism solving a problem the first one already solved. A non-decision once the precedent was checked.

**R3 — legal prose is restructured per-locale, not translated, because translating placeholder text has negative value.** `features/legal/policies/{en,fr}.tsx` gives every clone-owner one file to replace. The phase measured the placeholder legal corpus at roughly 2,000 words across `legal/` (~560 of them in `policies.config.tsx` alone) against the spec's estimate of "~57 blocks" — the disagreement didn't change the decision, it strengthened it: translating a boilerplate's fictional ToS/Privacy Policy is a legal act a clone-owner must redo in their own jurisdiction's language and wording, so doing it here would be work whose entire output gets discarded by every real deployment. `policies/fr.tsx` re-exports the English bodies verbatim.

**Every legal page discloses its untranslated body, not only `privacy`/`terms`.** Task 14's review found four more pages — `accessibility`, `cookies`, `data-rights`, `sub-processors` — shipping substantial English prose under already-translated French chrome, with no disclosure at all: 20 English strings on the RGPD walkthrough alone. R3's justification for leaving legal prose untranslated is the disclosure, not the omission — a French title over a silent English body is the "half a page is worse than none" defect this whole phase exists to prevent, even though the prose itself is legitimately in scope for R3. Fixed by extending the shared `UntranslatedBodyBanner` (already load-bearing on `privacy`/`terms`, since `requireCurrentPolicies` makes accepting the document mandatory) to all six legal pages, never by translating the ~560 words. The carve-out can now close honestly: every legal page either is translated, or tells the reader, in French, that it is not.

**R5 — an error formats to its final string immediately; a stale message already on screen does not re-translate if the user switches language mid-session.** The alternative — carry an `i18nKey` and resolve it at display time — was considered and rejected: it would have touched roughly 40 call sites, including screens this phase had already finished, for a benefit that only matters in the narrow window between a language switch and the next re-render of a stale error. The frozen-language trade-off is accepted as the cheaper, correct-enough choice.

**Three live defects the phase inherited, closed as a side effect of extraction rather than as its purpose** (commit `8ae9485`):
- `apps/app/src/features/api-tokens/api-tokens.schema.ts` — the `name` and `scopes` fields carried inline Zod `message:` strings (`"Name is required"`, `"Select at least one scope"`) that silently outrank the global `customError` map, since Zod gives a per-issue message absolute precedence. Both fields already had a translated key in the map; the inline literal was quietly winning over it, invisible because the screen was English throughout anyway.
- `apps/app/src/features/sso/sso.schema.ts` — `domain`, `issuer` and `entryPoint` validated through `.regex()`/`.startsWith()`, both of which raise Zod's `invalid_format` code, which can never carry `params.i18nKey` back to a specific catalog key — the global map can only answer it generically. Fixed via `.refine()`, whose `custom` issue code does carry `params.i18nKey`.
- `apps/app/src/features/admin-audit-log/components/audit-row.tsx` and `metadata-sheet.tsx` — `row.occurredAt as unknown as string` rendered the timestamp in the browser's own locale, unconditionally, regardless of the active language. Fixed by routing both through `useFormatDateTime` (commit `31bf02a`).

**The currency-formatting fix — the sibling of E.1a's `formatDate` promotion.** `apps/app/src/shared/components/pricing-table.tsx` built prices by hand (`` `${amount} ${CURRENCY}/${interval}` ``), which is wrong per locale by construction: English wants `$12/month`, French wants `12 €/mois`, and no single template-literal shape produces both. `formatPlanPrice` now goes through `Intl.NumberFormat(locale, { style: "currency", currency, maximumFractionDigits: 0 })` — the same "no date/currency library, `Intl` bound to an explicit locale argument" pattern E.1a established for `formatDate`/`formatDateTime` (commit `90b1302`).

**The parity gate grew by three assertions in Task 10 (17 → 20 tests), and they exist because a subagent volunteered a negative result about its own work.** The implementer reported — and the reviewer independently reproduced — that collapsing the French `expiryDays_one` onto its `_other` left both the en/fr-comparison suite and the completeness suite green: the gate compares the two locales against each other and is structurally blind to a catalog that is wrong *within* one locale. Only a human reading "1 jours" caught it. Three assertions now close that hole (`8377540`): no French `_one` may equal its `_other` outside an explicit `ALLOWED_SAME_PLURAL_FORM` list (English is deliberately exempt — its two forms are legitimately identical wherever the noun doesn't inflect around `{{count}}`); that list is swept for stale entries the same way `ALLOWED_IDENTICAL` is; and every plural must be declared in both forms, in both locales, or i18next silently falls back out of the missing bucket at runtime. The suite reached 21 independently, in Task 12, when the new `admin` namespace added its own entry to the per-namespace parity loop. Worth recording plainly: every other gate this phase added came from a defect that had already shipped. This one came from a defect caught before it shipped, because the implementer said so instead of reporting green and moving on.

**The inventory undercounted in five of the nine tasks where it was checked against the real file — always low, never high.** Task 5 (organization): 71 strings, not 70. Task 9 (webhooks): 71, not 70. Task 12 (admin users): 80 call sites, not 78. Task 13 (admin orgs / audit log): a fifth undercount, same shape. In each case the miss was not a quoted string the grep-based inventory failed to notice — it was a **bare enum variable interpolated raw into what became a translated sentence** (`invitation.role`, `d.status`, `user.role`, `item.role`, `member.role`, `row.actorType`): invisible to a literal-string grep because there is no literal to find, only a value the surrounding sentence now depends on. The recipe's Step F grew a second pass for exactly this after Task 5 shipped the first instance of it, and that second pass paid for itself on every task afterward. **The rule this leaves behind**: an inventory is a hypothesis about what a screen contains, not an authority on it. Grep the file before trusting a count, every time — the question that matters is not "how many quoted strings are there" but "how many places does English still reach the screen."

**Four follow-ups this phase deliberately did not close, recorded here so none reads later as an oversight:**

1. **Business codes for the 61 `HTTPException` call sites in `apps/api`** (verified: `grep -rn "new HTTPException" apps/api/src` still returns 61 at the end of this phase). Task 2 closed the user-visible half on the front — a `byStatus` layer in `messages.ts` covering all 61 sites by HTTP status; the API itself still answers `HTTP_<status>` where it should answer a domain code. Spec §4's "the API returns codes" is only half-held until the API side is done too.
2. **A French legal corpus.** `features/legal/policies/fr.tsx` re-exports the English bodies by design (R3). `requireCurrentPolicies` makes accepting the document mandatory, so the `UntranslatedBodyBanner` is a disclosure, not a substitute — whoever ships this to real French users must replace that one file.
3. **`EVENT_DESCRIPTIONS` stays English, permanently (R4).** It renders on `/settings/webhooks` via `event-type-picker.tsx`, directly beside the machine event names (`org.member.added`) that were never going to be translated either — that adjacency, decided in Task 9, is the justification: the row reads as technical data, not as untranslated prose.
4. **`domain-verification-card.tsx`'s dead branch, found in Task 11 and deliberately not fixed there.** The component tests `err.message === "DOMAIN_VERIFIED"` (`domain-verification-card.tsx:43`), but `@better-auth/sso` puts that string in `.code`, not `.message`, and `sso.mutations.ts` discards `.code` entirely. Re-verifying an already-verified domain therefore shows "DNS record not found" instead of the correct message — misleading, not crashing. Fixing it means threading `.code` through the mutation, a real behaviour change; burying that in a translation commit would have made the diff unreviewable.

**Three permanent exclusions, not two — the sub-processor register earns its own line.** Alongside the legal prose (R3) and `EVENT_DESCRIPTIONS` (R4), `apps/app/src/shared/sub-processors.config.ts` stays English because it restates the signed DPA — it is legal evidence, not display copy. But the two fields a user actually reads off it, `purpose` and `region`, are translated: they move through `common.legal.subProcessors.*`, named by `shared/sub-processor-labels.ts`, so `/settings/privacy`'s `DataSourcesCard` and the public `/legal/sub-processors` page both render French purpose/region text around an English-titled register. The exclusion covers the source-of-truth file, not what the user sees.

**Final gates, at `751d22c`.** `pnpm ci:check` (Biome) — 881 files, 0 fixes. `pnpm test` — api 800/800, app 279/279 (19/19 turbo tasks, matching Task 14's own count). `pnpm type-check` — 13/13 turbo tasks. `pnpm check:unused` — exit 0, 4 configuration hints, none i18n-related (the `POLICY_TYPES` warning is gone, as Task 14 expected). `pnpm check:duplication` — **34 clones / 483 duplicated lines / 1.13%**, against the measured baseline (dev @ `429d974`, before Task 1) of 35 clones / 496 duplicated lines / 1.24%. **The number moved down, not flat as expected.** The phase's working hypothesis going in — that jscpd would stay flat because it flags no i18n catalog pair, en and fr differing by construction in every value — held: nothing in the new catalog pairs shows up as a clone. The actual movement came from the opposite direction: extraction promoted a few small duplicated blocks to shared helpers along the way (`isHttpsUrl` to `shared/api/`, `role-labels.ts` to `shared/auth/`), which removed pre-existing clones elsewhere in files this phase was already touching — a side effect of the extraction, not a target of it. a11y gate: `docker compose up postgres -d` → `pnpm db:push && pnpm db:seed` → `pnpm --filter app build` (main chunk 257.14 kB, matching Task 14's own post-fix measurement — no export leak regression) → `pnpm --filter api dev` → `pnpm --filter app check:a11y` — **22/22**, matching Task 14's baseline.

**No new event types.** Nothing here mutates persistent business state — catalog stays **81 total / 35 public / 46 internal**, unchanged since Phase E.1a.

---

## Back ↔ front surface parity ✅ Phase H.1 · Sep 2026

**Why**: a boilerplate is judged on what the cloner can *do*, not on what the API can serve. A backend capability with no UI is invisible; a UI with no backend is a dead button. H.1 ran last on purpose — auditing the finished surface before E.1b would have meant translating screens twice.

**The map is a checked-in TypeScript artifact, not a hand-written table.** `apps/api/src/shared/surface/back-routes.ts` reads the route table off the live Hono app — 71 keys — rather than being maintained by hand alongside it, which would drift the moment one of the two changed without the other. Reading `app.routes` from a test required splitting app construction from boot: `apps/api/src/index.ts` previously ran `await di.preload()` and started the outbox dispatcher and three workers at module scope, so importing it booted the whole server. `apps/api/src/app.ts` now exports `app`/`routes`; `index.ts` keeps the boot. `front-consumers.ts` extracts 52 call sites from `apps/app`'s source. `route-map.ts` is the `Record<"METHOD /path", Consumer | UiLessReason>` over the 71 routes, with a runtime diff test (4 assertions) standing in for compiler exhaustiveness — routes have no hand-maintained union to `satisfies` against, since the true set only exists as `app.routes` at runtime. The template is the i18n parity test: flatten both sides, diff the sets, assert nothing is missing or stale.

**19 routes have no front consumer, and every one is declared, not silently absent.** 9 `internal-cron`, 3 `infra-probe`, 3 `public-api`, 2 `provider-callback`, 1 `library-owned` (the `/api/auth/*` wildcard — `better-call`'s internal route table is not exported, so it is documented as one opaque row rather than a wrong enumeration), 1 `dormant-by-design`. Nothing was deleted for being unconsumed — Axel: "faut être futur proof, si demain j'ai besoin du flux ça doit rester implémenté, le but c'est d'avoir tout de prêt, c'est un boilerplate." `POST /uploads/download` stays, declared dormant in writing. Capabilities with no HTTP surface at all (`quotas` — no `routes.ts`, never mounted) can never appear in `app.routes`, so the map is structurally incapable of carrying them; they get a short prose section in `docs/SURFACE.md` instead, honestly labelled as a human claim rather than a machine-verified one.

**Two gaps found, both confirmed and closed.** GAP 1: `PUT /admin/users/:id/role` was live, guarded and audited on the backend with no front consumer — a platform admin could not change a role from the UI. Now wired. GAP 2: replacing an avatar leaked the previous storage object — `createUpload` then `authClient.updateUser({ image })` never deleted the old one, and neither `POST /uploads/download` nor `DELETE /uploads` had a consumer. Fixed by deriving the object key server-side: `keyFromPublicUrl(url): Option<string>` on `IStorageService`, symmetric with `publicUrlFor`, and `deleteUploadBodySchema` now accepts `{ key }` *or* `{ url }`. The front must not learn the storage key format — `apps/app` has no `VITE_STORAGE_PUBLIC_URL`, and duplicating the public-URL base into the front is a config that drifts. A URL that isn't this storage's returns `{ deleted: false }`, a legitimate no-op — a social-login avatar is an ordinary `user.image` and must not raise.

**GAP 3 — the inverse direction (a front call site targeting a route that doesn't exist) — was INFIRMED.** A sweep of `disabled|TODO|FIXME|coming soon` over `apps/app/src` found only legitimate form states (`isPending`, `isFetchingNextPage`, current-plan). Nothing to fix; recorded here so a future reader doesn't re-run the audit.

**Behavioural parity — three refusal families didn't mirror, now two do.** `denyImpersonated` (~29 call sites across 9 modules) had no proactive front mirror: every guarded action stayed enabled, and the refusal only arrived as a 403 after the click. Seven call sites rendered raw backend English via `toast.error(err.message)` instead of the localized `formatApiError` path. Fixed on both legs, per the ROADMAP's own wording ("the action is hidden or disabled where it would be refused") — localized refusal copy (28 sites converted in total) *and* proactive disabling with the reason exposed without a mouse. The impersonation refusal gained its own business code: `deny-impersonated.middleware.ts` now throws `AppErrorException({ code: "IMPERSONATION_ACTION_FORBIDDEN" })` instead of a bare `HTTPException(403, ...)`, which previously resolved to the generic "You don't have permission to do this" — wrong, since the user does have permission, they're impersonating. Six of the nine refusal families already mirrored correctly and were left alone (auth/session, CSRF, rate-limit, `requirePlatformAdmin` + MFA, org capability, ownership scoping).

**`requireCurrentPolicies` mounted for real — reversing Phase A.2 decision 5.** A.2 shipped the middleware composable and unmounted, reasoning that a global mount "would make every current API call return 409 for a stale user — too aggressive." H.1 reverses that: a user who hasn't accepted the current policy version now gets `POLICY_ACCEPTANCE_REQUIRED` (409) on business routes, API tokens included. The refusal carries its own business code rather than a bare `HTTPException(409, ...)` — the same treatment the impersonation refusal got, and for the same reason: `HTTP_409` resolves to the generic "this action conflicts with the current state", which tells a user with stale terms nothing about what to do. The front handles the code globally (`apps/app/src/shared/api/errors/policy-refusal.ts`, wired where the router meets the query client): it refetches the policy status and invalidates the router, so `_shell`'s existing `shouldRedirectToLegalAccept` issues the redirect. Without it, a terms version published mid-session left whoever stayed on a settings page clicking Save against a wall — `beforeLoad` does not re-run on the page you are already on. **It is an allowlist, not a global mount with exclusions** — a distinction the first write-up of this phase got wrong, claiming "four exclusion classes" while three whole surfaces (`uploads`, `consents`, `/admin/*`) sat ungated and undeclared. The gate is declared route by route on the 19 mutating business routes of `profile`, `webhooks`, `api-token`, `organization`, `notifications`, `billing` and `uploads`, plus `PATCH /api/v1/me` — 20 mount sites in all. What is *not* gated, and why, is now enumerated in full: `POST /me/policies/accept` and `GET /me/policies` (the gate would otherwise block the only route that clears it); RGPD Art. 17/20 routes (a data-subject right cannot be conditioned on accepting terms); sign-out and session reads (a user must always be able to leave); `/consents`, which has no `requireAuth` at all — the cookie banner records for anonymous visitors, so the gate is structurally unmountable there, and withdrawing consent is the same family of right as RGPD anyway; the whole `/admin/*` surface, deliberately, because an operator whose own acceptance went stale would be 409-walled out of the console — `POST /admin/impersonation/stop` included, which is exactly the route they would need; impersonated sessions (`session.impersonatedBy` set — C.3 already skips the legal gate on the front during impersonation; the server had to agree or an operator would get 409-walled out of the account they're inspecting); and every read-only route. `uploads` was the one genuine omission rather than a deliberate exclusion — uploading is product usage, not a data-subject right, and nothing on `/legal/accept` uploads, so it could not deadlock the unblocking route. It is now gated on all four of its routes. Mounting it on `/api/v1` needed a typing change, not a second middleware: that surface runs on `ApiTokenVariables`, not `AuthVariables`, so `requireCurrentPolicies` was retyped against the minimal contract it actually reads — `{ user: SessionUser; session?: { impersonatedBy?: string | null } }` — making it mountable on both surfaces. A token request has no session, so the impersonation bypass is simply never taken there, which is correct: a token cannot impersonate.

**The E.1b plan's justification rested on a false premise — corrected, not the conclusion.** `docs/superpowers/plans/2026-08-31-e1b-i18n-extraction.md:67` claimed `requireCurrentPolicies` "blocks every protected route until the user accepts the current policy version," and used that claim to justify the `UntranslatedBodyBanner`. It was false when written (zero call sites) and would still be too broad today (RGPD rights, the acceptance route, and impersonated sessions are excluded). The line is rewritten to state the exclusion list, with a dated note pointing here. The plan's second reference (`:863`, "`requireCurrentPolicies` makes accepting the document mandatory") was equally false when written but is true as of this phase, and is left standing. The banner's justification survives either way — acceptance really is mandatory on the paths that matter — but a conclusion resting on a false premise is a defect even when the conclusion itself is right.

**One new event type — the first catalog movement since E.1a.** `notification.read` (internal, `operational` retention) is emitted when a user marks notifications read: the transition writes `read_at`, which is persistent state, and cross-cutting rule 6 admits no "small enough to skip" tier. Catalog now **82 total / 35 public / 47 internal**, verified against `packages/events/src/visibility-map.ts`. The event pulls double duty — it is also the signal the unread badge converges on, so the count stops waiting for the next poll to become right.

### Beyond the plan — the defects the audit surfaced

Auditing a finished surface finds what a plan written before the audit could not list. Everything below was found while executing the eleven planned tasks, or by Axel exercising the result in a browser, and every one of them was live on `dev`.

**2FA was dead, and it took the whole `/admin/*` console with it.** `packages/drizzle/src/schema/auth.ts` declared the `two_factor` table without two columns `better-auth@1.6.30` writes — `failed_verification_count` and `locked_until`, its verification-lockout state. Every 2FA verification 500'd, and since `requirePlatformAdmin` demands a fresh MFA challenge, nothing under `/admin/*` was reachable on an account with 2FA enabled. The subagent that found it correctly refused to patch product code under a translation-scoped brief; that refusal was overruled deliberately — a boilerplate shipping a dead security feature is worth more than a scope line. The other nine BetterAuth plugins were then audited for the same class of drift and are clean. **`db:push` fixing the dev box is not the fix**: the first review caught that the schema change shipped without a migration, so a fresh production database would have recreated the bug on day one — `packages/drizzle/migrations/0021_great_tana_nile.sql` is what actually closes it.

**Every modal could overflow, because no modal primitive declared a height.** The reported symptom was one broken screen (the webhook creation dialog, whose event-type picker lists the whole catalog). The cause was one level down: `Dialog`, `AlertDialog` and `Sheet` in `@packages/ui` set a width bound and no height bound, so any content taller than the viewport simply ran off the top and bottom of the screen with no way to scroll to it — 16 modals one long list away from the same bug. Fixed at the primitive (`max-h-[calc(100dvh-2rem)]` + `overflow-y-auto` on dialog and alert-dialog content, the same on the sheet's top/bottom sides and `overflow-y-auto` on left/right), which then made `delivery-sheet.tsx`'s own `overflow-y-auto` redundant and it was removed. **A layout bound that every call site must remember is a bug waiting for the next call site** — the same reason `apps/app/CLAUDE.md` keeps colors in the theme rather than at the point of use.

**The signature-verification example we hand to integrators was replayable.** `verify-snippet.tsx` is copy-paste code: it is what a customer's webhook endpoint will actually run. It parsed `t=<ts>` out of the header, folded the timestamp into the signed string — and never checked it. Since the signature covers the timestamp, a captured request stayed valid until the secret rotated. Now rejected outside a 300-second window. The `Number.isFinite` guard in front of the comparison is load-bearing, not defensive noise: `Number("abc")` is `NaN`, and `NaN > 300` is `false`, so without it a garbage timestamp passed the check that exists to stop it.

**`/api/v1` had no reference document.** The public surface is deliberately outside `AppType` — third parties do not get the Hono RPC client — which means it has no generated contract either, and its only description lived in `docs/FEATURES.md` as an inventory line. `docs/PUBLIC-API.md` is now the document a token holder reads: authentication, scopes, every route, the error-code table, and the webhook signature scheme. Every example in it was executed against a running instance rather than transcribed from the route handlers.

**Seven org-scoped queries shared a single cache entry across organizations.** The reported symptom was that a webhook created in Personal vanished when switching organization and needed a manual refresh to come back. The cause is that these endpoints are `requireOrg`: the server answers for the *active* organization, so a query key that does not name the organization asks TanStack Query to serve one organization's response for another's. With `staleTime: 30_000` and `refetchOnWindowFocus: false`, the stale entry is not merely shown — it is shown and kept. Fixed across `webhooks`, `api-tokens`, `current-membership`, `subscription`, `notifications`' org preferences and the rest, with `useActiveOrgId()` as the single place the id is read. **A cache key is a claim about what a response depends on**; if the server scopes a response by something the key omits, the cache is not a cache, it is a bug with a TTL.

**The notification rail carried two identical connection leaks, an inert selector, unreachable history, and a payload that shipped an invitation token to the browser.** The leaks are the same defect written twice — `notification-stream-hub.ts` and `outbox-dispatcher.service.ts` both registered `error` *and* `end` handlers that each called `scheduleReconnect()`, and neither ever disposed the dying `pg.Client`: no `removeAllListeners`, no `end()`, the field just overwritten. One dropped connection therefore produced two reconnects, and every previous client stayed alive holding a `LISTEN`. Both now share one in-flight reconnect timer, an idempotent disposer that removes listeners *before* ending the client, a field cleared only if it still points at the client that died, and a backoff reset only on a real `LISTEN` — plus, on the hub, `instrumentation.capture` on the failure path, which is how anyone would have seen this in production. The front got the other half: a stream that goes silent without erroring is now detected and reconnected. `NOTIFICATION_MAP` gained a `payloadFields` allowlist per event, because the SSE frame was forwarding the raw event payload — `org.member.invited` carries `invitationId`, which is the acceptance token, to every connected client of the organization. The email-frequency selector offered `immediate`/`hourly`/`daily` and did nothing with the answer; it now drives a digest window in SQL, with a preference flipped after a digest was scheduled honoured at flush time rather than at schedule time. The panel showed the most recent page with no way to reach any earlier one; it now loads more. And a read targeting someone else's notification answered success — it answers not-found now, which is the right shape for the same reason ownership scoping never answers 403: 403 confirms the row exists.

**Personal API tokens were invisible, and therefore unrevocable.** `routes.ts` read `session.activeOrganizationId ?? null` — but this stack always has an active organization (Personal is one), so the `?? null` branch was unreachable and the list only ever showed the active organization's tokens. A user's org-less tokens existed, authenticated, and could not be seen or revoked from the UI. `TokenOwner` is now a closed union (`{kind:"personal"}` | `{kind:"orgAndPersonal"}`) so the two cases cannot collapse into each other again, and `visibleTokensFilter` builds the `WHERE` from it. The filter is verified by `check:api-token-visibility` against a real database, not a mock, for the same reason as `check:fanout`: an `and/or/eq/isNull` tree is a value until Postgres evaluates it, and a mocked transaction asserts the shape of a query nobody ran.

**Ten tests were not running, and which ten depended on file order.** `bun test` shares one module registry across the whole run, so `mock.module` leaks process-wide: a stub registered in one file replaces the real module in every file that imports it afterwards. It bit three separate subagents in three unrelated disguises — hand-copied `@packages/events` catalogs blanking 17 event types in *other* files' assertions, an SQL-text assertion held hostage by another file's stubs, and a cross-suite `SyntaxError` that predated the branch. Measured rather than argued: `bun test src/modules/admin src/modules/api-token` ran 63 of 73 tests with 3 failures; the same command with `--isolate` runs 73 and passes. `--isolate --parallel` is now the default for every api test script, and `apps/api/src/shared/__TESTS__/module-isolation*` is a named canary over a module nothing else imports — one file replaces it, its sibling asserts it still sees the real one. Verified to fail without the flag. One consequence had to be paid for: pino ships `pino-pretty` to a worker thread, and a per-file registry spawns and tears one down per file, racing into "the worker thread exited" — so the logger is silenced under test.

**Final gates, at `834aaa9`.** `pnpm ci:check` (Biome) — 912 files, 0 fixes. `pnpm --filter api test` — **867/867** across 105 files, and green on three `--isolate --randomize` seeds (3, 11, 99), which is the assertion that matters after an isolation fix: order-independence is not provable by one ordering. `pnpm --filter app test` — 328/328 across 62 files. `@packages/i18n` — 21/21. `pnpm type-check` — 13/13 turbo tasks. `pnpm check:unused` — exit 0, 4 configuration hints, none introduced here. `pnpm check:duplication` — **35 clones / 517 duplicated lines / 1.15%**, against E.1b's 34 / 483 / 1.13%: one clone and 34 lines added across roughly 9,200 new lines, which is the shape you want — the percentage moved by two hundredths, not the direction.

**Seven executable checks now run against a real Postgres in CI**, not one: `check:sweep-lock`, `check:fanout`, `check:digest`, `check:api-token-visibility`, plus `check:wipe-rollback`, `check:enqueue` and `check:marksent`, which existed on this rail but had never been wired into the workflow — a check nothing runs is documentation with a shebang. They share a single rationale — each one's logic lives *inside* SQL (a lease's `FOR UPDATE SKIP LOCKED`, a preference cascade, a due-window filter, an ownership `WHERE` tree), and a mocked transaction can only assert that a query was built, never that Postgres agrees with what it means. `check:sweep-lock` refuses to run against a non-local database, because a check that acquires and releases leases is not something to point at production by accident.

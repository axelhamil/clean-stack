...
- **Per-recipient idempotency keys**: `EmailRecipient.idempotencyKey` lets a caller set a key per recipient; the positional fallback (`${options.idempotencyKey}#${index}`) is namespaced with `#` and shared by both the template batch path and the raw batch path. The digest flush route now keys its send on `notificationIds` rather than a per-call timestamp, making retries idempotent.
- **`failed` rows never purged**: `EMAIL_MESSAGE_FAILED_RETENTION_DAYS` (default 90) plus a new partial index (`email_message_failed_sweep_idx`) backs a second sweep pass. `sent` rows still purge on the original `EMAIL_MESSAGE_RETENTION_DAYS=7`.
- **`markSent` fan-out cost**: collapsed from one `UPDATE` per row to a single `UPDATE … WHERE id = ANY(...)` with a `CASE WHEN` selecting each row's `provider_message_id` — a 100-message chunk now costs one round-trip instead of 100.

**Supporting changes**: `runRetentionSweep` was promoted from a single pass to N passes, and all five existing callers (including `sweep-audit-log`) were migrated onto the promoted signature so the two email sweep passes (`sent`, `failed`) share the same runner as every other retention sweep in the codebase, rather than growing a bespoke second code path.

...
async function markReadFn(input: MarkReadInput): Promise<OkResponse> {
  ...
  if (!res.ok) await throwApiError(res, "Failed to mark notifications as read");
  return res.json();
}

async function markAllReadFn(): Promise<OkResponse> {
  const res = await $markAllRead({});
  if (!res.ok) await throwApiError(res, "Failed to mark all notifications as read");
  return res.json();
}
...
async function updateOrgPreferenceFn(input: UpdateOrgPreferenceInput): Promise<OkResponse> {
  ...
  if (!res.ok) await throwApiError(res, "Failed to update org notification preference");
  return res.json();
}

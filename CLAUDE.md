  ...
  app/
    src/
      ...
      features/<feature>/        One folder per sub-domain (NOT per area)
        <name>.route.tsx         Route definition: `export const <name>Route = createRoute({ getParentRoute, path, beforeLoad, validateSearch, component: lazyRouteComponent(...) })`. Parent imported from `router/layouts`. Lightweight — no JSX.
        <name>.page.tsx          Page component (separate file = code-split chunk). Accesses route's search/params/context via `getRouteApi("/path/id")`, never via static import of the route binding (cycle).
...
**Code-based routing on the front (no `routes/` folder).** TanStack Router's code-based API, canonical pattern: layouts/gates exported from `apps/app/src/router/layouts.tsx` (root, `_guest`, `_protected`, `_shell`, `_org-scope`, settings layout + index); each feature defines its route as a `const` directly importing its parent — `features/<feature>/<name>.route.tsx` does `export const <name>Route = createRoute({ getParentRoute: () => <parent>, ... })`. `apps/app/src/router.tsx` is a pure assembly file (~50 lines): imports layouts + feature routes, builds `routeTree` via `addChildren()`, exports `router`. **Why factory-less**: TanStack's typing requires the parent route reference at call site (not via generic factory) for the route's full path/context to be inferred. Per-feature gain over file-based: feature owns gate, search-schema, beforeLoad, component, params; removing a feature = `trash features/<x>/` + remove its line in `router.tsx` `addChildren`. Codegen-free (no `routeTree.gen.ts`, no Vite plugin watcher), TanStack Start migration is near-zero refactor (Start uses code-based as canonical).

**Code-splitting via `lazyRouteComponent` (the 2-file pattern).** Each route ships as a lazy chunk: `<name>.route.tsx` declares the route with `component: lazyRouteComponent(() => import("./<name>.page"), "<Name>Page")`; `<name>.page.tsx` exports the named Page component. **Why two files**: the bundler can only code-split a module reachable *exclusively* via dynamic `import()` — since `router.tsx` statically imports `<name>.route.tsx`, anything in that file lands in the main bundle. Page must live in a separate module. Trade-off accepted because the bundle saving is real (current floor ~588 KB initial vs ~760 KB merged). **Cycle avoidance**: pages access their route's `useSearch()` / `useParams()` / `useRouteContext()` via `getRouteApi("/full/path/id")` (string-based lookup, type-checked through the `Register` module declaration), NOT via a static `import { <name>Route } from "./<name>.route"` — that would create a `route ↔ page` cycle Biome flags. Default preload is `"intent"` so hover/focus prefetches the chunk before the user clicks.
...
- Static-import the route binding from a page file (`import { xxxRoute } from "./xxx.route"`) — creates a cycle Biome flags. Pages access their route via `getRouteApi("/path/id")`. The cycle is harmless at runtime (the page's import of the route is dynamic via `lazyRouteComponent`), but Biome's `noImportCycles` is still right to flag the static side — the `getRouteApi` pattern eliminates it cleanly.
- Merge a route file with its page file to "save a file" — code-splitting requires the page to live in a module ONLY reachable via dynamic `import()`. Merging puts the page in `router.tsx`'s static graph, busting the lazy chunk. The 2-file pattern is the SOTA trade-off; accept it.

...
- **Inside a translated namespace, no user-visible text is a literal at its call site.** The translated surface is what `packages/i18n/src/catalogs/en/` declares — today `common`, `auth`, `errors`, `emails`, `settings`, covering the auth flows, the account settings page and the shell. Inside it, every string that reaches the DOM, a `toast()`, an `aria-label` or an email subject comes from a catalog key through `t()`. **Why the boundary is drawn at the namespace and not at the whole app**: the API-tokens, SSO, admin and legal surfaces still ship literals *by design* (phase E.1b, in progress — the list shrinks as the phase lands and the carve-out goes away with its last task) — a rule the repo contradicts on every second file teaches people that CLAUDE.md is decorative, which costs more than the rule buys. **Why the rule at all**: a literal is invisible to both the type-checker and the parity gate, so it can never be translated and its absence can never be detected — it is a defect no CI run will ever surface. **Test before merging**: for a file whose feature is covered by a namespace, search the diff for a quoted string sitting in JSX text, in a `label`/`placeholder`/`title`/`aria-label` attribute, or in a `toast` argument. If one is there, it is wrong. English is the source of truth: add the key to `packages/i18n/src/catalogs/en/` first, or `tsc` rejects the call site.
- **Extending the translated surface is all-or-nothing per screen.** Translating half a page is worse than translating none of it: a French card sitting above an English one reads as a bug, not as work in progress. When a phase picks up a feature, its cards, its dialogs, its forms and the toasts its hooks raise move together, and the parity gate then holds both locales in step.

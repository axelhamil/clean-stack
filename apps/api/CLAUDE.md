...
**Defaults**: `session.cookieCache: { enabled: true, maxAge: 5*60 }` (signature-only check between DB refreshes; keep `maxAge` ≤ 15 min for near-instant revoke). `bearer()` alongside cookies — web uses cookies (httpOnly, XSS-safe), Capacitor uses bearer. Cookies: `httpOnly`, `secure: isProd`, `sameSite: isProd ? "none" : "lax"` — `none` because SPA+API are distinct origins; CSRF covered by `requireCsrf` (Origin allowlist). Same-site deploy → switch to `"lax"`.

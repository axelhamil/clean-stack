...
- **C.5** Webhooks **front UI** + `webhook.test` ✅ **shipped** (Jul 2026 — see ✅ table; `/settings/webhooks` operator page (CRUD, wildcard subscriptions, delivery timeline, per-attempt drawer, one-shot secret reveal, rotate, send-test, auto-disabled badge) + public `/developers/events` catalog (48 subscribable events, JSON schemas, Node verification snippet) + SSRF guard + dual-secret rotation + `webhook_delivery_attempt` table + auto-disable. 4 internal events → **52 total / 48 subscribable / 4 internal** *(counts as of C.5; the catalog is at 67 / 28 public / 39 internal today)*. As-built in [`docs/HISTORY.md`](docs/HISTORY.md). ~~**Next-step**: curate the public catalog~~ — **done in C.4**: `visibility-map.ts` made exposure an explicit allowlist, taking the public surface from "everything subscribable" down to 28.)
...
- **D.3** In-app notification center ✅ **shipped** (Aug 2026 — `modules/notifications/` back + `shared/notifications/` front; `<Bell />` inbox in the shell, `/settings/notifications` matrix, org defaults card; fan-out as an `OutboxSubscriber` **inside the dispatch TX, not the 1-line `onEvent(...)` this line originally promised** — `onEvent` is best-effort, so a lost notification fails silently; preference cascade (org lock → user → org default → enabled, `forced` bypassing all four) resolved inside the `INSERT ... SELECT`; SSE signal-only stream. **No new event type** — the catalog is consumed, staying **67 / 28 public / 39 internal**. As-built in [`docs/HISTORY.md`](docs/HISTORY.md).)
